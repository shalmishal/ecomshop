		  ...
		  <thead>
			...
			<tbody>
				<tr ng-repeat = "items in cart.cartItems">
					 	<td class="span1"><img src="G:\products/{{product.name}}.jpg" alt="image"/></td>
						<td>{{items.pname}}</td>
						<td>{{items.price}}</td>
...
<script>
 ...
 cartApp.controller("addToCartCtrl", function ($scope, $http){
     ...
     $scope.removeItemFromCart = function (id) {
         $http.put('http://localhost:8080/grostore/usercart/cart/removeItem/'+id).success(function (data) {
     ...
     $scope.addItemToCart = function (id) {
         $http.put('http://localhost:8080/grostore/usercart/cart/addItem/'+id).success(function (data) {
     ...
     $scope.refreshCartItems = function () {
         $http.get('http://localhost:8080/grostore/usercart/cart/refreshCart/'+$scope.cartId).success(function (data) {
            $scope.cart=data;
         });
     };
     ...
     $scope.clearCartItems = function () {
     	$http['delete']('http://localhost:8080/grostore/usercart/cart/clearCartItems/'+$scope.cartId).success(function (data)
     			{$scope.refreshCartItems()
     		});
     };
 });
